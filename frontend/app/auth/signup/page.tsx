      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-muted text-sm mt-1">Join CYVRA to start protecting yourself online</p>
          </div>
        </div>
            ...
            <div className="space-y-1.5">
              {passwordChecks.map(({ label, pass }) => (
                <div key={label} className="flex items-center gap-2">
                  <CheckCircle className={`w-3.5 h-3.5 ${pass ? "text-white" : "text-muted"}`} />
                  <span className={`text-xs ${pass ? "text-white" : "text-muted"}`}>{label}</span>
                </div>
              ))}
            </div>
          )}
